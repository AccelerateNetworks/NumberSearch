            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (ownedNumbersCycle != null && (ownedNumbersCycle.Enabled || ownedNumbersCycle.RunNow))
                    {
                        ...
                        if (lastRun != null && (lastRun.StartDate < (start - ownedNumbersCycle.CycleTime) || x > 0))
                        {
                            x--;
                            Log.Debug($"Last Run of {lastRun.IngestedFrom} started at {lastRun.StartDate} and ended at {lastRun.EndDate}");
                            ...
                            tasks.Add(
                                    Task.Run(async () =>
                                    {
                                        ...
                                        var combined = new IngestStatistics
                                        {
                                            StartDate = start,
                                            EndDate = DateTime.Now,
                                            FailedToIngest = billingClients.FailedToIngest,
                                            IngestedFrom = billingClients.IngestedFrom,
                                            IngestedNew = billingClients.IngestedNew,
                                            Lock = false,
                                            NumbersRetrived = billingClients.NumbersRetrived,
                                            Removed = 0,
                                            Unchanged = billingClients.Unchanged,
                                            UpdatedExisting = billingClients.UpdatedExisting,
                                            Priority = false
                                        };
